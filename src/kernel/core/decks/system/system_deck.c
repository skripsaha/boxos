...
static int system_deck_yield(Pocket* pocket) {
    if (!pocket) {
        return -1;
    }
...
static int system_deck_defrag_file(Pocket* pocket) {
    if (!pocket) {
        return -1;
    }
    ...
    if (!proc) {
        pocket->error_code = SYSTEM_ERR_PROCESS_NOT_FOUND;
        return -1;
    }

    defrag_file_event_t* req = vmm_translate_user_addr(proc->cabin,
                                                        pocket->data_addr,
                                                        pocket->data_length);
    if (!req) {
        pocket->error_code = SYSTEM_ERR_INVALID_ARGS;
        return -1;
    }
    ...
    defrag_file_response_t* resp = (defrag_file_response_t*)req;
    if (result == 0) {
...
static int system_deck_fragmentation_score(Pocket* pocket) {
    if (!pocket) {
        return -1;
    }
    ...
    fragmentation_score_response_t* resp = vmm_translate_user_addr(proc->cabin,
                                                                    pocket->data_addr,
                                                                    pocket->data_length);
    if (!resp) {
        pocket->error_code = SYSTEM_ERR_INVALID_ARGS;
    ...
    memset(resp, 0, sizeof(fragmentation_score_response_t));
...
static int system_deck_get_overflow_status(Pocket* pocket) {
    if (!pocket) {
        return -1;
    }
    pocket->error_code = 0;
...
int system_deck_handler(Pocket* pocket) {
    ...
    switch (opcode) {
        case SYSTEM_OP_PROC_SPAWN:
            return system_deck_proc_spawn(pocket);

        case SYSTEM_OP_PROC_KILL:
            return system_deck_proc_kill(pocket);

        case SYSTEM_OP_PROC_INFO:
            return system_deck_proc_info(pocket);

        case SYSTEM_OP_PROC_EXEC:
            return system_deck_proc_exec(pocket);

        case SYSTEM_OP_CTX_USE:
            return system_deck_ctx_use(pocket);

        case SYSTEM_OP_BYPASS:
            debug_printf("[SYSTEM_DECK] BYPASS_ON (0x05) not implemented (security feature)\n");
            pocket->error_code = SYSTEM_ERR_NOT_IMPLEMENTED;
            return -1;

        case SYSTEM_OP_BUF_ALLOC:
            return system_deck_buf_alloc(pocket);

        case SYSTEM_OP_BUF_FREE:
            return system_deck_buf_free(pocket);

        case SYSTEM_OP_BUF_RESIZE:
            return system_deck_buf_resize(pocket);

        case SYSTEM_OP_TAG_ADD:
            return system_deck_tag_add(pocket);

        case SYSTEM_OP_TAG_REMOVE:
            return system_deck_tag_remove(pocket);

        case SYSTEM_OP_TAG_CHECK:
            return system_deck_tag_check(pocket);

        case SYSTEM_OP_DEFRAG_FILE:
            return system_deck_defrag_file(pocket);

        case SYSTEM_OP_FRAGMENTATION_SCORE:
            return system_deck_fragmentation_score(pocket);

        case SYSTEM_OP_OVERFLOW_STATUS:
            return system_deck_get_overflow_status(pocket);

        case SYSTEM_OP_ROUTE:
            return system_deck_route(pocket);

        case SYSTEM_OP_ROUTE_TAG:
            return system_deck_route_tag(pocket);

        case SYSTEM_OP_LISTEN:
            return system_deck_listen(pocket);

        case SYSTEM_OP_YIELD:
            return system_deck_yield(pocket);
