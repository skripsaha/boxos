...
void test_listen_table(void) {
    ...
    // --- Test 2: Find listener by source_type ---
    total++;
    ...
    if (result == -BOXOS_ERR_LISTEN_ALREADY) {
        ...
        debug_printf("[TEST %d] FAIL: Expected -BOXOS_ERR_LISTEN_ALREADY, got %d\n", total, result);
    }

    // --- Test 4: Multiple source types ---
    total++;
    ...
    // --- Test 5: Find multiple listeners for same source type ---
    total++;
    ...
    // --- Test 7: Remove non-existent listener ---
    total++;
    ...
    // --- Test 8: cleanup_pid removes all entries for a PID ---
    total++;
    ...
    // --- Test 9: Fill table to MAX_LISTENERS ---
    total++;
    ...
    if (fill_ok) {
        ...
        if (result == -BOXOS_ERR_LISTEN_TABLE_FULL) {
            debug_printf("[TEST %d] PASS: Table full correctly reported\n", total);
            passed++;
        } else {
            debug_printf("[TEST %d] FAIL: Expected -BOXOS_ERR_LISTEN_TABLE_FULL, got %d\n", total, result);
        }
    }
...
void test_route_direct(void) {
    ...
    // --- Test 1: Successful direct route A -> B ---
    total++;
    ...
    kprintf("\n[TEST %d] Route to self (should fail ROUTE_SELF)\n", total);
    {
        ...
        int result = system_deck_route(&event);
        if (result == -1 && event.error_code == BOXOS_ERR_ROUTE_SELF) {
    ...
    kprintf("\n[TEST %d] Route to target=0 (should fail INVALID_ARGUMENT)\n", total);
    {
        ...
        int result = system_deck_route(&event);
        if (result == -1 && event.error_code == BOXOS_ERR_INVALID_ARGUMENT) {
    ...
    // --- Test 4: Route to non-existent PID ---
    total++;
    kprintf("\n[TEST %d] Route to non-existent PID (should fail PROCESS_NOT_FOUND)\n", total);
    {
        ...
        int result = system_deck_route(&event);
        if (result == -1 && event.error_code == BOXOS_ERR_PROCESS_NOT_FOUND) {
...
void test_route_tag(void) {
    ...
    kprintf("\n[TEST %d] Route by tag 'nonexistent' (no subscribers)\n", total);
    {
        ...
        int result = system_deck_route_tag(&event);
        if (result == -1 && event.error_code == BOXOS_ERR_ROUTE_NO_SUBSCRIBERS) {
...
void test_listen_via_system_deck(void) {
    ...
    // --- Test 1: Register keyboard listener ---
    total++;
    ...
    kprintf("\n[TEST %d] Register same listener again (should fail LISTEN_ALREADY)\n", total);
    {
        ...
        if (result == -1 && event.state == EVENT_STATE_ERROR) {
            ...
            if (err_code == (uint32_t)BOXOS_ERR_LISTEN_ALREADY) {
    ...
    // --- Test 3: Register a different source type ---
    total++;
