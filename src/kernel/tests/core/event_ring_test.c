...
static int test_push_pop_single(void) {
    ...
    error_t err = event_ring_push(ring, &evt);
    TEST_ASSERT(err == OK, "Push should succeed");
    ...
    TEST_ASSERT(err == OK, "Pop should succeed");
...
static int test_push_pop_multiple(void) {
    ...
    for (int i = 0; i < NUM_EVENTS; i++) {
        Event evt;
        error_t err = event_ring_pop(ring, &evt);
        TEST_ASSERT(err == OK, "Pop should succeed");
...
static int test_push_full(void) {
    ...
    for (int i = 0; i < 410; i++) {
        err = event_ring_pop(ring, &popped);
        TEST_ASSERT(err == OK, "Pop should succeed after auto-grow");
...
static int test_auto_grow(void) {
    ...
    for (int i = 0; i < 410; i++) {
        err = event_ring_pop(ring, &popped);
        TEST_ASSERT(err == OK, "Pop should succeed after growth");
...
static int test_multi_growth(void) {
    ...
    Event popped;
    for (int i = 0; i < 820; i++) {
        error_t err = event_ring_pop(ring, &popped);
        TEST_ASSERT(err == OK, "Pop should succeed");
...
static int test_max_capacity(void) {
    ...
    error_t err = event_ring_grow(ring);
    TEST_ASSERT(err == ERR_BUFFER_LIMIT_EXCEEDED, "Grow should fail at max capacity");
...
static int test_invalid_inputs(void) {
    ...
    error_t err = event_ring_push(NULL, &evt);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Push NULL ring should fail");

    err = event_ring_push(ring, NULL);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Push NULL event should fail");

    evt.magic = 0xDEADBEEF;
    err = event_ring_push(ring, &evt);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Push bad magic should fail");
    evt.magic = EVENT_MAGIC;

    err = event_ring_pop(NULL, &evt);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Pop NULL ring should fail");

    err = event_ring_pop(ring, NULL);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Pop NULL event should fail");

    err = event_ring_pop(ring, &evt);
    TEST_ASSERT(err == ERR_WOULD_BLOCK, "Pop empty ring should return WOULD_BLOCK");

    err = event_ring_grow(NULL);
    TEST_ASSERT(err == ERR_INVALID_ARGUMENT, "Grow NULL should fail");
...
static int test_wrap_around(void) {
    ...
    for (int i = 256; i < 512; i++) {
        evt.event_id = i;
        error_t err = event_ring_push(ring, &evt);
        TEST_ASSERT(err == OK, "Push should succeed during wrap");
    }
