...
uint64_t ReadEncoded(const uint8_t **p, uint8_t enc)
{
    ...
    uint64_t value;
    switch (enc & 0x0F) {
    case 0x00: value = ReadFixed<uint64_t>(p); break;
...
void EnsureIndexBuilt()
{
    ...
    if (__atomic_compare_exchange_n(&g_index_state, &expected, 1,
        ...
        BuildIndex();
        __atomic_store_n(&g_index_state, 2, __ATOMIC_RELEASE);
    } else {
                        ...
                        uint64_t cfa, bool cfa_valid)
{
    ...
    while (p < end) {
        ...
        if (op >= 0x30 && op <= 0x4F) {
            ...
            switch (op) {
            case 0x03: Push(ReadFixed<uint64_t>(&p)); break;
            case 0x06:
                Push(*reinterpret_cast<const uint64_t *>(Pop()));
                break;
            case 0x08: Push(ReadFixed<uint8_t>(&p)); break;
            case 0x0A: Push(ReadFixed<uint16_t>(&p)); break;
            case 0x0C: Push(ReadFixed<uint32_t>(&p)); break;
            case 0x0E: Push(ReadFixed<uint64_t>(&p)); break;
            case 0x10: Push(ReadUleb(&p)); break;
            case 0x11: Push(uint64_t(ReadSleb(&p))); break;
            case 0x12: { uint64_t v = Pop(); Push(v); Push(v); break; }
            case 0x13: Pop(); break;
            case 0x16: {
            ...
            case 0x1A: { uint64_t a = Pop(); Push(Pop() & a); break; }
            case 0x1C: { uint64_t a = Pop(); Push(Pop() - a); break; }
            case 0x1E: { uint64_t a = Pop(); Push(Pop() * a); break; }
            case 0x21: { uint64_t a = Pop(); Push(Pop() | a); break; }
            case 0x22: { uint64_t a = Pop(); Push(Pop() + a); break; }
            case 0x23: Push(Pop() + ReadUleb(&p)); break;
            case 0x24: { uint64_t a = Pop(); Push(Pop() << a); break; }
            case 0x25: { uint64_t a = Pop(); Push(Pop() >> a); break; }
            case 0x9C:
...
bool DwarfFindFrame(uint64_t pc, FrameInfo *out)
{
    ...
    p += 4;
    (void)ReadEncoded(&p, out->cie.fde_enc);
