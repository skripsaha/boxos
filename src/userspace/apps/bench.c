...
{
    if (n == 0) {
        out->iters = 0; out->ok = false; return;
    }
    ...
    uint64_t sum = 0;
    for (uint32_t i = 0; i < n; i++) sum += deltas_tsc[i];
...
static void emit_num_padded(char *buf, int *pos, int field, uint64_t v)
{
    ...
    if (v == 0) { tmp[len++] = '0'; }
        ...
        while (v > 0 && rl < 23) { rev[rl++] = (char)('0' + (v % 10)); v /= 10; }
        for (int i = 0; i < rl; i++) tmp[len++] = rev[rl - 1 - i];
    }
    int pad = field - len;
    while (pad-- > 0) buf[(*pos)++] = ' ';
...
static void emit_str_padded(char *buf, int *pos, int field, const char *s)
{
    ...
    for (int i = 0; i < copy; i++) buf[(*pos)++] = s[i];
    for (int i = copy; i < field; i++) buf[(*pos)++] = ' ';
}
...
static bench_stats_t bench_run_raw(bench_fn fn, void *ctx, uint32_t n)
{
    ...
    for (uint32_t i = 0; i < WARMUP_ITERS; i++) {
        if (fn(i, ctx) < 0) { st.ok = false; st.err = -1; return st; }
    }
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (rc < 0) { st.ok = false; st.err = rc; return st; }
...
static int b_rdtsc_overhead(uint32_t i, void *ctx) { (void)i; (void)ctx; return 0; }
...
static int b_cabin_info(uint32_t i, void *ctx)
{
    ...
    if ((volatile uint32_t)ci->magic == 0) return 0;
    return 0;
}
...
static int b_display_ping(uint32_t i, void *ctx)
{
    ...
    if (rc < 0) return rc;
    Result r;
    if (!receive_wait(&r, 2000)) return -ERR_TIMEOUT;
    return 0;
}
...
static int b_chain_bulk(uint32_t i, void *ctx)
{
    ...
    if (!s_bulk_built) {
        ManifestBuilder mb;
        if (ManifestBuilderInit(&mb, s_bulk_mbuf, sizeof(s_bulk_mbuf)) != 0) return -1;
        for (int k = 0; k < BULK_OPS; k++) {
            uint8_t fb = (uint8_t)k;
            if (ManifestBuilderAddOp(&mb, DECK_OPERATIONS, DECK_OPS_FILL, 0,
                                     CRATE_INDEX_NONE, 0, &fb, 1) != 0) return -1;
        }
        if (ManifestBuilderFinalize(&mb) != 0) return -1;
        s_bulk_built = true;
    }
    Crate c;
    ...
    if (rc != 0 || r.error_code != 0) return -1;
    return 0;
}
...
static int b_file_cycle(uint32_t i, void *ctx)
{
    ...
    uint32_t v = i;
    if (v == 0) { rev[rl++] = '0'; }
    else { while (v > 0 && rl < 11) { rev[rl++] = (char)('0' + (v % 10)); v /= 10; } }
    while (rl > 0) name[p++] = rev[--rl];
    name[p] = '\0';
    ...
    if (wrc < 0) { delete((uint32_t)file_id); return wrc; }
...
{
    char line[120]; int p = 0;
    while (*label) line[p++] = *label++;
    line[p++] = ' ';
    char rev[24]; int rl = 0;
    uint64_t x = v;
    if (x == 0) rev[rl++] = '0';
    else { while (x > 0 && rl < 23) { rev[rl++] = (char)('0' + (x % 10)); x /= 10; } }
    while (rl > 0) line[p++] = rev[--rl];
    if (unit && *unit) { line[p++] = ' '; while (*unit) line[p++] = *unit++; }
    line[p] = '\0';
    println(line);
}
