...
**Location:** `src/kernel/core/events/events.h`
...
**Location:** `src/kernel/core/ipc/notify_page.h`

```c
typedef struct __packed {
    uint32_t magic;                          // 4 bytes: 0x4E4F5449 "NOTI"
    uint8_t  prefix_count;                   // 1 byte
    uint8_t  flags;                          // 1 byte: NOTIFY_FLAG_CHECK_STATUS, NOTIFY_FLAG_YIELD
    uint8_t  status;                         // 1 byte: NOTIFY_STATUS_OK/RING_FULL/INVALID
    ...
    uint32_t spawner_pid;                    // 4 bytes: parent process PID
    uint8_t  _reserved[7854];               // 7854 bytes: pad to 8192
} notify_page_t;                             // TOTAL: 8192 bytes (8KB, 2 pages)
```
...
NOTIFY_FLAG_YIELD         = 0x80  // Cooperative yield: no event, just reschedule
```

## Result Page Structure

**Location:** `src/kernel/core/ipc/result_page.h`

```c
typedef struct __packed {
    uint8_t  source;                   // 1 byte: ROUTE_SOURCE_KERNEL/PROCESS/HARDWARE
    uint8_t  _reserved1;               // 1 byte
    uint16_t size;                     // 2 bytes
    error_t error_code;                // 4 bytes
    uint32_t sender_pid;               // 4 bytes: PID of sender (IPC)
    uint8_t  payload[RESULT_PAYLOAD_SIZE]; // 244 bytes
...
1. **Prefix Chain:** Maximum 16 prefixes per event
   ...
   - Terminator 0x0000 is added after last prefix if space available
...
- Event structure: `src/kernel/core/events/events.h`
- Notify page: `src/kernel/core/ipc/notify_page.h`
- Result page: `src/kernel/core/ipc/result_page.h`
- Userspace API: `src/userspace/boxlib/include/box/types.h`
- Cabin layout: `src/include/cabin_layout.h`
- Size constants: `src/include/boxos_sizes.h`
- Guide dispatcher: `docs/core/guide.md`
