...
0x3000 - 0xBFFF          36KB     Result Page           Read-Only  Кольцевой буфер ответов от ядра
0xC000 - ...             var      Code + Data + BSS     R / RW     Тело программы и глобальные переменные
...
3. Структура Result Page (0x3000 - 0xBFFF, 36KB)
...
typedef struct __packed {
    ...
    struct {
        ...
        uint8_t _pad2[60];
        result_entry_t entries[128];         // Кольцо из 128 результатов
    } ring;
    uint8_t _reserved[56];
} result_page_t;                             // TOTAL: 36864 bytes (9 pages)
```
...
Никаких скрытых состояний: Всё общение с ОС происходит через две области памяти (Notify 8KB + Result 36KB). Для отладки достаточно посмотреть дамп памяти по адресам 0x1000 и 0x3000.
