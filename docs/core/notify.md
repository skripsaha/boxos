...
Synchronous Yield:
   ...
   Результат: Процесс отдаёт CPU. Guide обрабатывает событие. Процесс получает результат через Result Page (0x3000-0xBFFF) когда планировщик вернёт ему управление.
...
4. Структура взаимодействия
   ...
   | Результат -> Result Page (0x3000, 36KB)
   V
