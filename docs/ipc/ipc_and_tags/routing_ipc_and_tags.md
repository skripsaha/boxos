...
#### notify_page_t (0x1000 - 0x2FFF, 8KB)
...
typedef struct __packed {
    ...
    uint8_t  reserved1;
    uint16_t prefixes[16];                   // Цепочка Snowball (макс 16 префиксов)
    uint8_t  data[256];                      // Полезная нагрузка
    ...
    uint32_t route_target;                   // PID получателя (0 = доставить себе)
    char     route_tag[32];                  // Тег для fan-out ("" = не используется)
...
typedef struct __packed {
    ...
    uint32_t sender_pid;                   // PID отправителя (0 = kernel)
    uint8_t  route_flags;                  // ROUTE_SOURCE_KERNEL/PROCESS/HARDWARE
...
| Вписывается в философию | ✅ | "Снежный ком" катится через деки — ROUTE просто ещё один дек в цепочке. Command pipeline модель сохранена. Notify/Result модель не нарушена. |
| Человечность | ✅ | Программист видит print(), input(), send(). Не видит prefixes, ROUTE, ResultPage. boxlib скрывает всё. |
| Без больших минусов | ✅ | Компромиссов нет. ResultEntry.payload = 244 байта. Достаточно для большинства IPC сообщений. |
