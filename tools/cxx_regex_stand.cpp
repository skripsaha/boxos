...
// A copy of boxcxx's std::regex_traits<char>, over the host's locale rather
...
public:
    ...
    static std::size_t length(const char *p) { return std::strlen(p); }
    ...
    template <class It> string_type lookup_collatename(It first, It last) const
    {
        ...
        for (const auto &e : kNames)
            if (name == e.n) return string_type(1, e.v);
        return string_type();
    }
    ...
    bool isctype(char c, char_class_type f) const
    {
        ...
        if ((f & kUnderscore) && c == '_') return true;
        return false;
    }

    int value(char ch, int radix) const
    {
        int v;
        if (ch >= '0' && ch <= '9') v = ch - '0';
