...
run_config() {
    ...
    if [ "$MODE" = "fast" ]; then
        burst="memtest files bench mtest chain cow_test lifecycle write_observer decks htest cxxtest current_test"
    else
        burst="memtest files bench memtest files bench memtest \
               mtest chain htest cxxtest current_test cow_test lifecycle write_observer \
